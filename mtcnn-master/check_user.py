...
while(1):
    ...
    for i in range(len(detect_result)):
        ...
        cv2.putText(image, f"{i} : {detect_result[i]['radianText']}", (detect_result[i]['box'][0], detect_result[i]['box'][1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2, cv2.LINE_AA)
                    ...
                    for z in range(len(check_total_radian)):
                        if check_total_radian[z]['sNum'] == user_box[i]['description']:
                            try:
                                ...
                                if (check_total_radian[z]['radian'][0] - 5 < detect_result[j]['radianValue']) & (check_total_radian[z]['radian'][0]+5 > detect_result[j]['radianValue']):
